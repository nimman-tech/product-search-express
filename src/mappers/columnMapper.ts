...
export interface ColumnMapper {
  ...
  /**
   * Returns true when the given API or DB column represents a boolean field.
   */
  isBooleanColumn(columnName: string): boolean;

  /**
   * Normalize DB values to match the API contract.
   */
  normalizeValue(columnName: string, value: unknown): unknown;
...
abstract class BaseColumnMapper implements ColumnMapper {
  ...
  constructor(columns: Record<string, string>, booleanColumns: string[] = []) {
  ...
  isBooleanColumn(columnName: string): boolean {
    return this.booleanColumns.has(this.mapColumn(columnName));
  }

  normalizeValue(columnName: string, value: unknown): unknown {
    if (!this.isBooleanColumn(columnName) || value === null || value === undefined) {
      return value;
    }
...
class CarColumnMapper extends BaseColumnMapper {
  constructor() {
    super(
      ...
      [
        'feature_sunroof',
        'feature_ventilated_seats',
        'feature_wireless_charger',
        'feature_reverse_camera',
        'feature_view360',
        'feature_watch_connect',
        'infotainment_available',
  ...
  constructor() {
    super(
      ...
      [
        ...
        'battery_removable',
        'features_face_unlock',
        'features_fingerprint_sensor',
        'features_always_on_display',
        'features_de_x_support',
        'features_stylus_support',
        'features_fast_charging',
        'features_reverse_charging',
        'features_audio_jack',
        'features_stereo_speakers',
        'features_hi_res_audio',
        'features_gps',
        'features_nfc',
        'features_infrared',
        'features_support5g',
